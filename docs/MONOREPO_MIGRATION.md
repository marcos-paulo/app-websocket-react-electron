...
- `shared` → `@app/shared`
- `backend` → Agora usa `@app/shared` automaticamente
- `frontend-redux` → Agora usa `@app/shared` automaticamente
...
Instala tudo em: /node_modules/
    ↓
Cria symlink: /node_modules/@app/shared → ../../shared
    ↓
...
npm link @app/shared     # ❌ Não precisa
